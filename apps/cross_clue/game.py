...
class CrossClueGameManager(BaseGameManager):
    ...
    def update_config(
        self, session_id: str, turn_timer: int = 60, game_timer: int = 300
    ) -> CrossClueGameState:
    ...
    def submit_clue(self, session_id: str, user_id: str, clue: str) -> bool:
        ...
        if (
            game_state.status != "playing"
            or game_state.active_giver_id != user_id
            or not game_state.active_turn
        ):
            return False
    ...
    def guess_coordinate(
        self, session_id: str, user_id: str, guess: str
    ) -> Optional[Dict]:
        ...
        if (
            game_state.status != "playing"
            or game_state.active_guesser_id != user_id
            or not game_state.active_turn
        ):
            return None
    ...
    def _is_game_complete(self, game_state: CrossClueGameState) -> bool:
        ...
        revealed_count = len(
            [
                coord
                for coord, state in game_state.grid_state.items()
    ...
    def game_timeout(self, session_id: str) -> Optional[Dict]:
        ...
        if game_state.active_turn and "secret_coordinate" in game_state.active_turn:
            ...
            if secret not in game_state.grid_state:
                ...
                if game_state.guess_history is None:
                    game_state.guess_history = {}
                game_state.guess_history[secret] = (
                    game_state.active_guesser_id
                )  # Current guesser gets credit for attempt
    ...
    def submit_vote(
        ...
        if (
            user_id == game_state.active_giver_id
            or user_id == game_state.active_guesser_id
        ):
            return None
    ...
    def return_to_lobby(self, session_id: str, user_id: str) -> CrossClueGameState:
        ...
        if user_id in game_state.players:
            ...
            if game_state.status == "finished":
                ...
                game_state.players[user_id].has_submitted = (
